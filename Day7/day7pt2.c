...
node* smallestAbove(node* start, size_t min) {
    size_t best = nodeSize(start);
    if(best < min) return NULL;
    node* sel = start;
    for(int i = 0; i < start->data.d.n_contents; i++) {
        node* n = start->data.d.contents[i];
        if(!n->dir) continue;
        node* cand = smallestAbove(n, min);
        if(!cand) continue;
        size_t nsz = nodeSize(cand);
        if(nsz >= min && nsz < best) {
            sel = cand;
            best = nsz;
        }
...
int main() {
    ...
    size_t fullsize = nodeSize(&root);
    size_t target = fullsize - 40000000;
    size_t realval = nodeSize(smallestAbove(&root, target));
    printf("From %lu delete %lu - %lu\n", fullsize, target, realval);
}